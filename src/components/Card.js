...
function Card() {
  ...
  const lastName = useSelector(selecUserLastName)
  const userId = useSelector(selecUserDocument)
  const [cards,setCards] = useState();

  useEffect(() => {
    const getCard = async() => {
      await axios.get(`http://localhost:8080/accounts`,{params: {
        userId: userId,
    }})
      .then((response) => {
  ...
  return (
    ...
    {cards &&
      cards.map((card) => (
        <FlipCard key={card.id} onMouseEnter={handleFlip} onMouseLeave={handleFlip}>
